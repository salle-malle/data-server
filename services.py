    ...
    for ticker_symbol in tickers:
        try:
            ...
            for news_item in news_list[:5]:
                try:
                    url = news_item.get('link') or news_item.get(
                        'content', {}).get('canonicalUrl', {}).get('url')
                    pub_date = news_item.get('providerPublishTime') or news_item.get(
                        'content', {}).get('pubDate')
            ...
            logger.info(
                f"'{ticker_symbol}' 뉴스 크롤링 완료. {len(crawled_articles)}개 기사 수집.")
        except Exception as e:
...
def crawl_yahoo_stock_market_news(limit: int = 10) -> Dict[str, List[NewsArticle]]:
    ...
    try:
        ...
        for selector in article_selectors:
            ...
            for link in links:
                ...
                try:
                    ...
                    # 기사 본문 추출
                    article = Article(
                        article_url, language='en', config=config)
                    article.download()
                    article.parse()
                    ...
                    if article.publish_date:
                        try:
                            ...
                            if article.publish_date.tzinfo is None:
                                utc_time = article.publish_date.replace(
                                    tzinfo=ZoneInfo("UTC"))
                            else:
                                utc_time = article.publish_date.astimezone(
                                    ZoneInfo("UTC"))
                            kst_time = utc_time.astimezone(
                                ZoneInfo("Asia/Seoul"))
                            date_kst = kst_time.strftime('%Y-%m-%d %H:%M:%S')
                        except Exception:
                            # 날짜 파싱 실패시 현재 시간 사용
        ...
        logger.info(
            f"Yahoo Finance 주요 증시뉴스 크롤링 완료: {len(crawled_articles)}개 기사 수집")
